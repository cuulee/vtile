...
func TileXYToBounds(id TileID) Bounds {
	tileX := id.x
	tileY := id.y
	levelOfDetail := id.z
	...
	return Bounds{s: float64(minLat), n: float64(maxLat), e: float64(maxLon), w: float64(minLon)}
}

func Get_XY(lat, long float64, levelOfDetail float64) TileID {
	lat_rad := lat * (math.Pi / 180.0)
	n := math.Pow(2.0, levelOfDetail)
	xtile := int64((long + 180.0) / 360.0 * n)
	ytile := int64((1.0 - math.Log(math.Tan(lat_rad)+(1/math.Cos(lat_rad)))/math.Pi) / 2.0 * n)
	ztile := uint64(levelOfDetail)
	return TileID{x: xtile, y: ytile, z: ztile}
}
